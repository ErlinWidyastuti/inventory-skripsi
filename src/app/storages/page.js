...
export default function Storages() {
  ...
  // Ambil ulang seluruh data storage setelah tambah/edit/hapus
  ...
  // Fungsi untuk membuka modal tambah storages baru
  const handleAdd = () => {
    setName("");
    setEditId(null);
    setError(null);
  ...
  // Tambah atau update storage (data lokasi penyimpanan)
  const handleAddOrUpdateStorage = async (e) => {
    e.preventDefault();
    setLoading(true);
    setError(null);
    setSuccess(null);
    ...
    try {
      ...
      if (editId) {
        ...
        setSuccess("Lokasi Penyimpanan berhasil diperbarui!");
      } else {
        // Tambah data satuan baru untuk input ke supabase
        const { error } = await supabase.from("storages").insert([{ name }]);
        if (error) throw error;
  ...
  const handleDelete = async (id, name) => {
    if (confirm('Yakin ingin menghapus lokasi penyimpanan ini?')) {
      try {
        const { error } = await supabase.from('storages').delete().eq('id', id);
        if (error) throw error;
        setSuccess('Lokasi Penyimpanan berhasil dihapus!');
        fetchStorages();
      } catch (error) {
        if (error.message.includes("violates foreign key")) {
          setError(`Lokasi Penyimpanan "${name}" tidak dapat dihapus karena digunakan pada data barang masuk.`);
        } else {
          setError(error.message);
        }
      }
      ...
      <div className="flex-1 p-8">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-3xl font-bold text-foreground mb-2">
            Lokasi Penyimpanan - Selamat Datang, {user?.username}!
          </h1>
        </div>
        <div className="bg-white p-6 rounded-lg shadow-md mb-6">
          <div className="flex justify-between items-center mb-4">
            <div>
              <h2 className="text-2xl font-semibold text-gray-700">Daftar Lokasi Penyimpanan</h2>
              <p className="text-base text-gray-600">Kelola data lokasi penyimpanan</p>
            </div>
            <button
              onClick={handleAdd}
            ...
            >
              <Plus size={22}/> Tambah Lokasi Penyimpanan
            </button>
          </div>
          ...
          {/* Tabel data storages */}
          {storages.length === 0 ? (
            <p className="text-gray-700">Belum ada lokasi penyimpanan.</p>
          ) : (
            <table className="w-full table-auto">
              ...
              <tbody>
                {storages.map((storage) => (
                  <tr key={storage.id} className="border-b">
                    <td className="px-4 py-2 text-gray-700">{storage.name}</td>
        ...
        {/* Modal tambah dan edit storages */}
        {isModalOpen && (
